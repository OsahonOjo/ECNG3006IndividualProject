 ...
 * Hence, the highest number of maxima in this test is 20 / 3 = 6.667;
 *
 * Test cases:
 * - TC1: 0 maxima
 * - TC2: 2 maxima
    ...
    while (1)
    {
        ...
        if (xQueueReceive(adc_readings_queue, adc_readings_arr, 0))
        {
            ...
            for (int i = 1; i < N_MAXIMA_UPPER_LIMIT; i++)
            {
                ...
                ESP_LOGI(TAG, "Interval: %i, Indices: %i - %i, Period: %i", nn_intervals_arr[i-1], maxima_indices_arr[i], maxima_indices_arr[i-1], SAMPLING_PERIOD_IN_MS);
            }
            ...
            for (int i = 0; i < N_MAXIMA_UPPER_LIMIT - 1; i++)
            {
                ...
                ESP_LOGI(TAG, "Current NN Interval Total: %i", nn_avg);
            }
        ...
        {
            ESP_LOGI(TAG, "Queue is empty. Beginning task delay...");
            vTaskDelay(EFFECTIVE_PERIOD_IN_MS);
            ESP_LOGI(TAG, "Delay period elapsed. Ending task delay...");
        }
...
void app_main(void)
{
    ...
    pulse_rate_queue = xQueueCreate(10, sizeof(uint16_t));//chng 10->2
    ...
    /*while(1)
    {
        ESP_LOGI(TAG, "In app_main infinite loop");
    }*/
